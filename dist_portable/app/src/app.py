...
if 'saved_config' not in st.session_state: st.session_state.saved_config = ConfigService.load_config()
...
def render_dashboard():
    ...
    # Varsayilan olarak sadece Fiziksel portlar (Sayısal kodlar guncellendi: 0 ve 4 eklendi, 1 cikarildi)
    PHYSICAL_TYPES = ['physical', 'hard-switch', 'fsw', 'root', '0', '4']

    interfaces = api.get_interfaces(sel_dev, vdom=sel_vdom)
    
    # --- YETKI KONTROLU (Port Bazli) ---
    user = AuthService.get_current_user()
    if user and user.username != "admin":
        cfg = ConfigService.load_config()
        acc = next((x for x in cfg.get("local_accounts", []) if x['user'] == user.username), None)
        
        # Eger kullanicinin port kisitlamasi varsa (Global veya Cihaz Bazli)
        if acc:
            global_ports = acc.get("global_allowed_ports", [])
            specific_ports = acc.get("allowed_ports", {}).get(sel_dev) # None ise ozel tanim yok

            # Kısıtlama var mi?
            # 1. Global liste doluysa KISITLAMA VARDIR.
            # 2. Specific liste None degilse (bos olsa bile) KISITLAMA VARDIR.
            has_restriction = (len(global_ports) > 0) or (specific_ports is not None)
            
            if has_restriction:
                allowed_set = set(global_ports)
                if specific_ports:
                    allowed_set.update(specific_ports)
                
                if allowed_set:
                    interfaces = [i for i in interfaces if i['name'] in allowed_set]
                else:
                    # Kısıtlama aktif ama izin seti boş
                    interfaces = []

                if not interfaces:
                    st.warning("⚠️ Bu cihazda görüntüleme yetkiniz olan port bulunmamaktadır.")
                    return
    # -------------------------------------
    ...
    if interfaces:
        ...
        for i in interfaces:
            ...
            # Checkbox kapaliysa sadece fiziksel tipleri goster
        ...
        if not filtered_interfaces:
            st.warning("Görüntülenecek port bulunamadı.")
